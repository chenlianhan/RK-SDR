...
void write_memcpy_data(uint16_t *d) {
    FILE *fw = fopen("data3.bin", "wb");
    for (d; d < d + DATA_BUFFER-1; d++)
    {
        fwrite(d, sizeof(uint16_t), 1, fw);
    }
...
void signal_generate(struct arguments *arguments) {
    ...
    for(i = 0; i < DATA_BUFFER / 2 - 1; i++) {
        ...
        printf2(I_amp);
        // printf2(*(arguments->data + i*2));
        data[i * 2 + 1] = Q_amp;
        // *(arguments->data + i*2 + 1)=Q_amp;
        printf2(Q_amp);
        // printf2(*(arguments->data + i*2 + 1));
        // printf("%d\n",i);
...
int main(int argc, char *argv[]) {
    ...
    arguments->data = (uint16_t*)malloc(sizeof(uint16_t)*DATA_BUFFER);
